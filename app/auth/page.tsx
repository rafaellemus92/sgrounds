...
function AuthForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: err } = await supabase.auth.signInWithOtp({
      email,
      options: {
        emailRedirectTo: 'https://sgrounds.vercel.app/auth/callback',
      },
    })
    if (err) {
      ...
      {sent ? (
        <div className="text-center animate-slideUp">
          <p className="font-display text-[18px] italic" style={{ color: 'rgba(var(--sg-text-rgb), 0.7)' }}>
            Check your email — link expires in 24 hours.
          </p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          {linkExpired && (
            <p className="text-[13px] font-body text-center mb-2" style={{ color: 'rgba(201, 169, 110, 0.85)' }}>
              Your link expired — enter your email for a new one.
            </p>
          )}
          <input
            type="email"
